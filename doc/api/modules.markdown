...
JXcore has a simple module loading system.  In JXcore, files and modules are in
...
JXcore has several modules compiled into the binary.  These modules are
...
If the exact filename is not found, then JXcore will attempt to load the
...
and does not begin with `'/'`, `'../'`, or `'./'`, then JXcore starts at the
...
`require('bar.js')`, then JXcore would look in the following locations, in
this order:
...
This is the extent of JXcore's awareness of package.json files.

If there is no package.json file present in the directory, then JXcore
...
of absolute paths, then JXcore will search those paths for modules if they
are not found elsewhere.  (Note: On Windows, `NODE_PATH` is delimited by
semicolons instead of colons.)

Additionally, JXcore will search in the following locations:

* 1: `$HOME/.node_modules`
* 2: `$HOME/.node_libraries`
* 3: `$PREFIX/lib/node`

Where `$HOME` is the user's home directory, and `$PREFIX` is JXcore's
...
When a file is run directly from JXcore, `require.main` is set to its
`module`. That means that you can determine whether a file has been run
directly by testing

    require.main === module

For a file `foo.js`, this will be `true` if run via `jx foo.js`, but
...
The semantics of JXcore's `require()` function were designed to be general
enough to support a number of sane directory structures. Package manager
programs such as `dpkg`, `rpm`, and `npm` will hopefully find it possible to
build native packages from JXcore modules without modification.
...
Since JXcore looks up the `realpath` of any modules it loads (that is,
