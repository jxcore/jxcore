...
JXcore statically compiles all its dependencies into the executable.
...
Note that all JXcore addons must export an initialization function:
...
You can now use the binary addon in a JXcore project `hello.js` by pointing `require` to
...
prototype:
    ...
    JX::ThreadStore<Persistent<Function> > MyObject::c_constructor;
    ...
    void MyObject::Init(Handle<Object> exports) {
      ...
      const int tid = c_constructor.getThreadId();
      c_constructor.templates[tid] = Persistent<Function>::New(tpl->GetFunction());
      exports->Set(String::NewSymbol("MyObject"), c_constructor.templates[tid]);
    }
    ...
    Handle<Value> MyObject::New(const Arguments& args) {
      ...
      if (args.IsConstructCall()) {
        ...
        const int tid = c_constructor.getThreadId();
        return scope.Close(c_constructor.templates[tid]->NewInstance(argc, argv));
      }
    }
...
Or test it under multitasking:
    ...
    var obj = {
        ...
        logic: function () {
            var obj = new addon.MyObject(10);
            console.log(obj.plusOne(), process.threadId); // 11
            console.log(obj.plusOne(), process.threadId); // 12
            console.log(obj.plusOne(), process.threadId); // 13
        }
    };
...
The implementation is similar to the above in `myobject.cc`:
    ...
    Handle<Value> MyObject::NewInstance(const Arguments& args) {
      ...
      const int tid = c_constructor.getThreadId();
      Local<Object> instance = c_constructor.templates[tid]->NewInstance(argc, argv);
...
by unwrapping them with JXcore's `node::ObjectWrap::Unwrap` helper function.
