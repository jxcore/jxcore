...
exports.saveBatchFile = function(script) {
  ...
  fs.writeFileSync(fname, script.replace(/JX_BINARY/g, '"' + process.execPath + '"'));
...
exports.onlyForMT = function() {

  if (!exports.mted && !exports.mtkeeped) {
    jxcore.utils.console.warn('This test is designed only for mt/mt-keep.');
    process.exit();
    return true;
  }
};
...
exports.onlyForSingleThread = function() {

  if (exports.mted || exports.mtkeeped) {
    jxcore.utils.console.warn('This test is designed only for single thread.');
    process.exit();
    return true;
  }
};
...
exports.checkMinimumVersion = function(versionString) {

  var v = process.jxversion.replace(/v\s/gi, '').trim();
  var v2 = versionString;

  if (v === v2)
    return true;

  // cut first version number "0."
  if (v.slice(0,2) === '0.') v = v.slice(2);
  if (v2.slice(0,2) === '0.') v2 = v2.slice(2);


  if (semver.lt(v, v2)) {
    jxcore.utils.console.info('Current jx version:', process.jxversion);
    jxcore.utils.console.warn('This test can run only for jx', versionString, 'and above');
    return false;
    process.exit();
  }
