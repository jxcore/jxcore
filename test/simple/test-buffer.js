...
assert.equal(new Buffer('abc').toString({
  toString: function() {
    return 'ascii';
  }
}), 'abc');
...
var expectedIllegal = expected.slice(0, 60) + ' \x80' + expected.slice(60, 120)
        + ' \xff' + expected.slice(120, 180) + ' \x00'
        + expected.slice(180, 240) + ' \x98' + expected.slice(240, 300)
        + '\x03' + expected.slice(300, 360);
...
assert
        .equal(new Buffer('KioqKioqKioqKio=', 'base64').toString(),
                '***********');
...
assert.equal(new Buffer('72INjkR5fchcxk9+VgdGPFJDxUBFR5/rMFsghgxADiw=',
        'base64').length, 32);
assert
        .equal(new Buffer('72INjkR5fchcxk9+VgdGPFJDxUBFR5/rMFsghgxADiw',
                'base64').length, 32);
assert.equal(new Buffer('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg==',
        'base64').length, 31);
assert
        .equal(new Buffer('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg=',
                'base64').length, 31);
assert
        .equal(new Buffer('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg',
                'base64').length, 31);
...
['hex', 'utf8', 'utf-8', 'ascii', 'binary', 'base64', 'ucs2', 'ucs-2',
    'utf16le', 'utf-16le'].forEach(function(enc) {
...
[16, 32].forEach(function(bits) {
  ...
  }, /beyond buffer length/, 'readUInt' + bits + 'LE');

  assert.throws(function() {
    buf['readInt' + bits + 'BE'](0);
  }, /beyond buffer length/, 'readInt' + bits + 'BE()');

  assert.throws(function() {
    buf['readInt' + bits + 'LE'](0);
  }, /beyond buffer length/, 'readInt' + bits + 'LE()');
});
...
assert.throws(function() {
  ...
  for (var i = 0; i < len; ++i)
    buf[i] = 0x42; // Try to force segfault.
}); // , /RangeError/
