...
/*
 This code compares process.env called from main thread and method.runOnce() as method
 */
...
var method = function (obj) {
  ...
  process.sendToMain(JSON.stringify(process.env));
};
...
jxcore.tasks.on('message', function (tid, ret) {
  ...
  if (value !== ret) {
    ...
    assert.strictEqual(value, ret, "Values (main thread vs thread " + process.threadId + ") are not equal: process.env");
  }
