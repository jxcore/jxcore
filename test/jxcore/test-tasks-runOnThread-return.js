...
jxcore.tasks.runOnThread(0, method, 1, function (ret) {
  var expected = 1;
...
jxcore.tasks.runOnThread(1, method, "2", function (ret) {
  var expected = "2";
  assert.strictEqual(expected, ret, "method(" + expected + ") received wrong value: " + JSON.stringify(ret) + " instead of " + JSON.stringify(expected));
  finished++;
});
...
jxcore.tasks.runOnThread(2, method, "three", function (ret) {
  var expected = "three";
  assert.strictEqual(expected, ret, "method(" + expected + ") received wrong value: " + JSON.stringify(ret) + " instead of " + JSON.stringify(expected));
  finished++;
});
...
jxcore.tasks.runOnThread(3, method, {four: 1}, function (ret) {
...
jxcore.tasks.runOnThread(4, method, [5], function (ret) {
