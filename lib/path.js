...
if (isWindows) {
  ...
  var splitPath = function(filename) {
    ...
    var result = splitDeviceRe.exec(filename), device = (result[1] || '')
            + (result[2] || ''), tail = result[3] || '';
  ...
  exports.resolve = function() {
    ...
    for (var i = arguments.length - 1; i >= -1; i--) {
      ...
      if (i >= 0) {
        ...
        if (!path
                || path.substr(0, 3).toLowerCase() !== resolvedDevice
                        .toLowerCase()
                        + '\\') {
          path = resolvedDevice + '\\';
        }
      ...
      var result = splitDeviceRe.exec(path), device = result[1] || '', isUnc = device
              && device.charAt(1) !== ':', isAbsolute = !!result[2] || isUnc, // UNC
      // paths
      // are
      // always
      // absolute
  ...
  exports.join = function() {
    ...
    return exports.normalize(paths.filter(
            function(p, index) {
              if (typeof p !== 'string') { throw new TypeError(
                      'Arguments to path.join must be strings'); }
              return p;
