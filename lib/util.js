...
function formatValue(ctx, value, recurseTimes) {
  ...
  !(value.constructor && value.constructor.prototype === value)) { return String(value
  ...
  if (keys.length === 0) {
    ...
    if (isRegExp(value)) { return ctx.stylize(RegExp.prototype.toString
            .call(value), 'regexp'); }
    if (isDate(value)) { return ctx.stylize(
  ...
  if (keys.length === 0 && (!array || value.length == 0)) { return braces[0]
          + base + braces[1]; }
...
function formatArray(ctx, value, recurseTimes, visibleKeys, keys) {
  ...
  keys.forEach(function(key) {
    if (!key.match(/^\d+$/)) {
      output.push(formatProperty(ctx, value, recurseTimes, visibleKeys, key,
              true));
    }
  });
  return output;
 ...
 * (not on Function.prototype). NOTE: If this file is to be loaded during
 * bootstrapping this function needs to be rewritten using some native functions
 * as prototype setup using normal JavaScript does not work as expected during
