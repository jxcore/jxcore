...
function Zlib(opts, mode) {
  ...
  if (opts.flush) {
    if (opts.flush !== binding.Z_NO_FLUSH
            ...
            && opts.flush !== binding.Z_BLOCK) { throw new Error(
  ...
  if (opts.chunkSize) {
    if (opts.chunkSize < exports.Z_MIN_CHUNK
            || opts.chunkSize > exports.Z_MAX_CHUNK) { throw new Error(
            'Invalid chunk size: ' + opts.chunkSize); }
  }

  if (opts.windowBits) {
    if (opts.windowBits < exports.Z_MIN_WINDOWBITS
            || opts.windowBits > exports.Z_MAX_WINDOWBITS) { throw new Error(
            'Invalid windowBits: ' + opts.windowBits); }
  }

  if (opts.level) {
    if (opts.level < exports.Z_MIN_LEVEL || opts.level > exports.Z_MAX_LEVEL) { throw new Error(
            'Invalid compression level: ' + opts.level); }
  }

  if (opts.memLevel) {
    if (opts.memLevel < exports.Z_MIN_MEMLEVEL
            || opts.memLevel > exports.Z_MAX_MEMLEVEL) { throw new Error(
  ...
  if (opts.strategy) {
    if (opts.strategy != exports.Z_FILTERED
            ...
            && opts.strategy != exports.Z_DEFAULT_STRATEGY) { throw new Error(
            'Invalid strategy: ' + opts.strategy); }
  }

  if (opts.dictionary) {
    if (!Buffer.isBuffer(opts.dictionary)) { throw new Error(
  ...
  this._binding.init(opts.windowBits || exports.Z_DEFAULT_WINDOWBITS, level,
          opts.memLevel || exports.Z_DEFAULT_MEMLEVEL, strategy,
          opts.dictionary);
...
Zlib.prototype._transform = function(chunk, encoding, cb) {
  ...
  if (this._closed) return cb(new Error('zlib binding closed'));
  ...
  function callback(availInAfter, availOutAfter, buffer) {
    ...
    if (availOutAfter === 0) {
      ...
      var newReq = self._binding.write(flushFlag, chunk, inOff, availInBefore,
