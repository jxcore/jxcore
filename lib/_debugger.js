...
var util = require('util'), path = require('path'), net = require('net'), vm = require('vm'), repl = require('repl'), inherits = util.inherits, spawn = require('child_process').spawn;
...
exports.start = function(argv, stdin, stdout) {
  ...
  var args = ['--debug-brk'].concat(argv), interface_ = new Interface(stdin,
          stdout, args);
...
Protocol.prototype.execute = function(d) {
  ...
  case 'body':
    ...
    if (resRawByteLength - this.bodyStartByteIndex >= this.contentLength) {
      ...
      res.body = buf.slice(this.bodyStartByteIndex,
      ...
      this._newRes(buf.slice(this.bodyStartByteIndex + this.contentLength)
              .toString('utf8'));
    }
    break;
...
Client.prototype.reqLookup = function(refs, cb) {
  ...
  this.req(req, function(err, res) {
    if (err) return cb(err);
    for ( var ref in res) {
      if (res.hasOwnProperty(ref) && typeof res[ref] == 'object') {
...
var commands = [
    ['run (r)', 'cont (c)', 'next (n)', 'step (s)', 'out (o)',
        'backtrace (bt)', 'setBreakpoint (sb)', 'clearBreakpoint (cb)'],
    ['watch', 'unwatch', 'watchers', 'repl', 'restart', 'kill', 'list',
...
function SourceUnderline(sourceText, position, repl) {
  ...
  var head = sourceText.slice(0, position), tail = sourceText.slice(position);
...
function Interface(stdin, stdout, args) {
  ...
  var proto = Interface.prototype, ignored = ['pause', 'resume', 'exitRepl',
      'handleBreak', 'requireConnection', 'killChild', 'trySpawn',
      'controlEval', 'debugEval', 'print', 'childPrint', 'clearline'], shortcut = {
    'run': 'r',
    'cont': 'c',
    'next': 'n',
    'step': 's',
...
function leftPad(n, prefix) {
  var s = n.toString(), nchars = intChars(n), nspaces = nchars - s.length - 1;
...
Interface.prototype.list = function(delta) {
  ...
  var self = this, client = this.client, from = client.currentSourceLine
          - delta + 1, to = client.currentSourceLine + delta + 1;
          ...
          .reqSource(
                  ...
                  function(err, res) {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                      ...
                      var current = lineno == 1 + client.currentSourceLine, breakpoint = client.breakpoints
                              .some(function(bp) {
                                return (bp.scriptReq === client.currentScript || bp.script === client.currentScript)
                                        && bp.line == lineno;
                              });
                      ...
                      if (current) {
                        line = SourceUnderline(lines[i],
                                client.currentSourceColumn, self.repl);
                      } else {
                        line = lines[i];
                      }

                      self.print(leftPad(lineno, breakpoint && '*') + ' '
                              + line);
                    }
                    self.resume();
                  });
...
Interface.prototype.backtrace = function() {
  ...
  client.fullTrace(function(err, bt) {
    ...
    if (bt.totalFrames == 0) {
      ...
      var trace = [], firstFrameNative = bt.frames[0].script.isNative;
...
Interface.prototype.scripts = function() {
  ...
  var client = this.client, displayNatives = arguments[0] || false, scripts = [];

  this.pause();
  for ( var id in client.scripts) {
    if (!client.scripts.hasOwnProperty(id))
      continue;
    var script = client.scripts[id];
    if (typeof script == 'object' && script.name) {
      if (displayNatives || script.name == client.currentScript
              || !script.isNative) {
        scripts.push((script.name == client.currentScript ? '* ' : '  ') + id
...
Interface.prototype.watchers = function() {
  var self = this, verbose = arguments[0] || false, callback = arguments[1]
          || function() {
          }, waiting = this._watchers.length, values = [];
...
Interface.prototype.setBreakpoint = function(script, line, condition, silent) {
  ...
  var self = this, scriptId, ambiguous;
...
Interface.prototype.clearBreakpoint = function(script, line) {
  ...
  var ambiguous, breakpoint, index;

  this.client.breakpoints.some(function(bp, i) {
    if (bp.scriptId === script || bp.scriptReq === script
  ...
  if (breakpoint === undefined) { return this.error('Script : ' + script
          + ' not found'); }
...
Interface.prototype.pause_ = function() {
  ...
  var self = this, cmd = 'process._debugPause();';
...
Interface.prototype.trySpawn = function(cb) {
  var self = this, breakpoints = this.breakpoints || [], port = exports.port, host = 'localhost', childArgs = this.args;
  ...
  var client = self.client = new Client(), connectionAttempts = 0;
