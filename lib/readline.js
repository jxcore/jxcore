...
function Interface(input, output, completer, terminal) {
  if (!(this instanceof Interface)) { return new Interface(input, output,
          completer, terminal); }
  ...
  if (typeof completer !== 'function') { throw new TypeError(
          'Argument \'completer\' must be a function'); }
...
Interface.prototype._setRawMode = function(mode) {
  if (typeof this.input.setRawMode === 'function') { return this.input
          .setRawMode(mode); }
};
...
Interface.prototype._tabComplete = function() {
  ...
  self.completer(self.line.slice(0, self.cursor), function(err, rv) {
    ...
    var completions = rv[0], completeOn = rv[1]; // the text that was completed
...
Interface.prototype._ttyWrite = function(s, key) {
  ...
  if (key.ctrl && key.shift) {
    ...
    if (this._sawReturn && key.name !== 'enter') this._sawReturn = false;
...
/*
 * Some patterns seen in terminal key escape codes, derived from combos seen at
 * http://www.midnight-commander.org/browser/lib/tty/key.c
 * 
 * ESC letter ESC [ letter ESC [ modifier letter ESC [ 1 ; modifier letter ESC [
 * num char ESC [ num ; modifier char ESC O letter ESC O modifier letter ESC O 1 ;
 * modifier letter ESC N letter ESC [ [ num ; modifier char ESC [ [ 1 ; modifier
 * letter ESC ESC [ num char ESC ESC O letter - char is usually ~ but $ and ^
 * also happen with rxvt - modifier is 1 + (shift * 1) + (left_alt * 2) + (ctrl *
 * 4) + (right_alt * 8) - two leading ESCs apparently mean the same as one
 * leading ESC
 */
...
function emitKey(stream, s) {
  ...
  if (s === '\r') {
    ...
    var code = (parts[1] || '') + (parts[2] || '') + (parts[4] || '')
            + (parts[6] || ''), modifier = (parts[3] || parts[5] || 1) - 1;
...
/**
 * clears the current line the cursor is on: -1 for left of the cursor +1 for
 * right of the cursor 0 for the entire line
 */
