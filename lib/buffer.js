...
function Buffer(subject, encoding, offset) {
  ...
  if (typeof offset === 'number') {
    if (!Buffer.isBuffer(subject)) { throw new TypeError(
    ...
    if (typeof subject !== 'number') {
      if (type === 'string') {
        ...
        if (subject.parent)
          subject.parent.copy(this.parent, this.offset, subject.offset,
...
function isArrayIsh(subject) {
  return Array.isArray(subject) || subject && typeof subject === 'object'
...
Buffer.prototype.indexOf = function indexOf(val, byteOffset) {
  ...
  if (typeof val === 'string')
    return SlowBuffer.indexOfString(this, val, byteOffset);
  if (val instanceof Buffer)
    return SlowBuffer.indexOfBuffer(this, val, byteOffset);
  if (typeof val === 'number')
    return SlowBuffer.indexOfNumber(this, val, byteOffset);
...
Buffer.prototype.inspect = function inspect() {
  var out = new Array(), len = this.length, name = this.constructor.name;
...
Buffer.prototype.compare = function compare(b) {
  ...
  return SlowBuffer.compare(this, b);
};
...
Buffer.byteLength = function(str, enc) {
  if (!enc || enc == 'utf8') { return SlowBuffer.Utf8Length(str, enc); }
  var ret;
  str = str + '';
  switch (enc) {
  case 'ascii':
  ...
  case 'hex':
    ...
    break;
  default:
    ret = SlowBuffer.byteLength(str, enc);
  }
  return ret;
};
...
Buffer.prototype.fill = function fill(value, start, end) {
  ...
  if (typeof value !== 'number' || isNaN(value)) { throw new TypeError(
...
Buffer.prototype.copy = function(target, target_start, start, end) {
  ...
  if (end < start) throw new RangeError('sourceEnd < sourceStart');
  ...
  return this.parent.copy(target.parent || target, target_start
          + (target.offset || 0), start + this.offset, end + this.offset);
};
...
/*
 * Need to make sure that buffer isn't trying to write out of bounds. This check
 * is far too slow internally for fast buffers.
 */
...
function checkInt(buffer, value, offset, ext, max, min) {
  ...
  if ((offset % 1) !== 0 || offset < 0)
    throw new TypeError('offset is not uint');
  if (offset + ext > buffer.length || buffer.length + offset < 0)
    throw new RangeError('Trying to write outside buffer length');
}
