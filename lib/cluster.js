...
cluster.setupMaster = function(options) {
  ...
  if (execArgv.some(function(s) {
    return /^--prof/.test(s);
  }) && !execArgv.some(function(s) {
    return /^--logfile=/.test(s);
  })) {
  ...
  if (!options.exec && process._MTED) {
    settings.exec = process.argv[2];
  }
...
function isInternalMessage(message) {
  return isObject(message) && typeof message.cmd === 'string'
  ...
  Worker.prototype.disconnect = function() {
    ...
    sendInternalMessage(this, {
      ...
      for ( var key in serverListeners) {
        if (!serverListeners.hasOwnProperty(key))
          continue;
