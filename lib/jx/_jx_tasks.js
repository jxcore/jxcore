...
var gcc = function (arr) {
  ...
  for (var o in arr) {
    if (o == null || Array.prototype[o]) continue;
...
exports._addTask = function (method, param, cb) {
  ...
  if(arguments.length > 3) {
    ...
    var msg = '(jxcore.tasks) warning: Passing obj arguments when ' +
      'creating tasks has been deprecated. See https://goo.gl/dtWqpX.';
    ...
    var msg = '(jxcore.tasks) warning: Task callback functions ' +
      'now take two arguments (err, result). See https://goo.gl/dtWqpX.';
    if (process.throwDeprecation)
      throw new Error(msg);
    else if (process.traceDeprecation)
      console.trace(msg);
