// Copyright & License details are available under JXCORE_LICENSE file

var uw = process.binding("thread_wrap");

var markers = {};
var objects = {};
var events = {
  'emptyQueue': [],
  'message': []
};
var trackerId = 0;

var cpuCount = 2;
var cpuSet = false;
var exiting = false;

exports.setThreadCount = function(count) {
  if (process.subThread) { throw new Error(
          "You can not change the thread count under a subthread."); }
  // TODO(obastemur) use commons.h MAX_JX_THREADS
  if (count < 2 || count > 64) { throw new Error(
          "setThreadCount - min 2, max 64"); }

  if (!process.__tasking) {
    cpuSet = true;
    cpuCount = count + 1;
  }
};

exports.killThread = function(threadId, keep_execution) {
  if (threadId < 0 || threadId > 63) { throw new RangeError(
          "Thread Id - min 0, max 63"); }

  uw.killThread(threadId, keep_execution);
};

exports.unloadThreads = function(already_exiting) {
  if (process.subThread) { throw new Error(
          "You can not unload the threads under a sub thread. Try killThread."); }

  exports.runOnce(function() {
    process.unloadThread();
  }, null, true);

  if (!already_exiting) {
    setTimeout(function() {
      // dummy wait
    }, 200);
  }
};

var gcc_alive = false;

exports.begin = function() {
  if (exiting) return;
  if (process.subThread) { throw new Error(
          "You can not modify thread pool under a subthread."); }

  if (!process.__tasking) {
    process.on('exit', function() {
      exiting = true;
      exports.unloadThreads(1);
      uw.setProcessExiting(false);
    });

    // TODO(obastemur) get the number from commons.h MAX_JX_THREADS
    if (cpuCount > 64) {
      cpuCount = 64;
    }
    uw.setCPUCount(cpuCount);
    process.__tasking = true;
  }
};

exports.jobCount = function() {
  return uw.jobsCount();
};

exports.forceGC = function() {
  uw.freeGC();
};

var dummy = function() {
  if (!exiting) {
    gcc_alive = uw.jobsCount() > 0;

    if (!gcc_alive) {
      if (cinter != null) {
        clearInterval(cinter);
        cinter = null;
      }

      if (trackerId != 0) {
        trackerId = 0;
        cbb = 0;
        markers = {};
        if (events['emptyQueue'].length) {
          var arr = events['emptyQueue'];
          for ( var o in arr) {
            if (o != null) arr[o]();
          }
        }
      }
    }
  }
};

var cinter = null;
var cbb = 0;

var runCinter = function() {
  if (cinter != null || exiting) return;
  cinter = setInterval(function() {
    if (cbb >= trackerId) {
      dummy();
    }
  }, 250);
};

var emitMessage = function(obj) {
  var t = (obj.threadId || obj.threadId === 0) ? obj.threadId : obj.tid;
  var d = (obj.params || obj.params === 0) ? obj.params : obj.data;
  exports.emit('message', t, d);
};

var gcc = function(arr) {
  if (exiting) { return; }

  for ( var o in arr) {
    if (o == null) continue;

    var obj = JSON.parse(arr[o]);

    if (!obj || obj._id == -2)// runOnce
    {
      cbb++;
      if (obj) delete obj;
      continue;
    }

    if (obj.threadId || obj.threadId === 0 || obj.tid || obj.tid === 0) {
      if (obj.resetMe) {
        cbb += obj.counter;

        // give a chance calling method to exit
        cbb--;

        var tot = 100;
        if (obj.threadId > 0) tot = 175 * (obj.threadId);

        setTimeout(function() {
          uw.resetThread(obj.threadId);
          setTimeout(function() {
            if (!process._MTED) {
              cbb++;
            }
          }, 1000);// TODO: fix this.. ugly and unreliable. keeps main thread
          // alive until sub restart.
        }, tot);
      } else if (obj.wait) {
        cbb -= obj.wait;
      } else if (events['message'].length) {
        emitMessage(obj);
      }
      delete obj;
      continue;
    }

    if (obj.reset) {
      delete obj;
      continue;
    }

    var cb = (obj._id || obj._id === 0) ? markers[obj._id] : 0;
    cbb++;
    if (!cb && obj.dummy) {
      delete obj;
      continue;
    }

    if (cb) {
      if (obj.dummy) {
        cb();
      } else if (!objects[obj._id]) {
        cb(obj.o);
      } else {
        cb(objects[obj._id], obj.o);
        delete objects[obj._id];
      }
      delete markers[obj._id];
    }
    delete obj;
  }
  delete arr;
  arr = null;
};

exports.getThreadCount = function() {
  if (!process.__tasking && !process.subThread) { return (cpuSet)
          ? cpuCount - 1 : 2; }

  return uw.getCPUCount();
};

exports.on = function(event, cb) {
  if (!event || !event.substr) { throw new Error("expects event_name as string"); }

  var ee = events[event];
  if (ee) {
    ee.push(cb);
  }
};

exports.emit = function(event, p1, p2, p3) {
  if (events[event].length) {
    var zarr = events[event];
    for ( var zo in zarr) {
      zarr[zo](p1, p2, p3);
    }
  }
};

var taskId = 1;

var isFunction = function(method) {
  return Object.prototype.toString.call(method) == '[object Function]';
};

var _activated = false;

var getMethod = function(method) {
  var mt;
  if (!method) { throw new TypeError("jxcore.tasks requires a function body"); }

  if (!_activated) {
    _activated = true;
  }

  if (method.define || method.logic) {
    if (!method.define) { throw new Error("undefined ::logic for the task"); }
    if (!method.logic) {
      method.logic = function() {
      };
    }
    if (!isFunction(method.define) && !method.define.substr) { throw new Error(
            "jxcore.tasks requires a function body"); }
    var def = method.define.toString();
    var ind = def.lastIndexOf('return ');
    if (ind > 0) {
      var q = def.indexOf('}', ind);
      var z = def.indexOf('}', q + 1);
      if (z < 0) {
        if (def.indexOf(';', ind) > 0)
          console.log('Warning! task.define can not return!');
      }
    }
    mt = method.logic.toString() + '$$&JX;1$node$&JX;$' + def
            + '$$&JX;2$node$&JX;$' + method.waitLogic;
  } else {
    if (!isFunction(method)) { throw new Error(
            "jxcore.tasks requires a function body"); }
    mt = method.toString();
  }

  return mt;
};

/**
 * Adds new task to be processed in a separate thread. The task will be
 * processed as soon as possible. If there is any idle subthread, it will
 * execute the task immediately. Otherwise it may wait until the other tasks
 * will finish or some of the subthreads will become idle.
 * 
 * @param method
 *          {function} - This is the method, which will be executed in a
 *          subthread.
 * @param script
 *          {object} - Argument for that method.
 * @param cb
 *          {function} - It is a callback function, which will be invoked after
 *          the method completes.
 * @param obj
 *          {object}
 * @example var method = function (obj) { console.log("We are in a subthread
 *          now."); return "ok"; }; tasks.addTask(method, {count: 1000},
 *          function (result) { console.log("This is result from subthreaded
 *          method:", result); });
 */
exports.addTask = function(method, script, cb, obj) {
  if (process.subThread) { throw new Error(
          "You can not add a task under a subthread."); }

  exports.begin();
  if (script == undefined) {
    script = null;
  }
  if (method._taskId) {
    addTask(method._taskId, null, script, cb, obj);
  } else {
    method._taskId = taskId;
    taskId++;
    var mt = getMethod(method);

    addTask(method._taskId, mt, script, cb, obj);
  }

  if (cinter == null) {
    gcc_alive = true;
    runCinter();
  }
};

var addTask = function(taskId, method, param, cb, obj) {
  var cbId = trackerId;

  if (cb) {
    markers[cbId] = cb;
    objects[cbId] = obj;
  } else {
    cbId = -1;
  }

  // TODO if null!!! should work
  param = JSON.stringify(param);
  var err = uw.addTask(taskId, method, param, cbId, false);

  if (err > 0) { throw new Error("Thread creation error. id:" + err); }

  trackerId++;
};

/**
 * Adds new task to be processed just once, but for every existing subthread.
 * This means, that every subthread will call the task once. Every subthread
 * will process his task as soon as possible. If there are no other tasks in the
 * subthread's queue, the task will be executed immediately. Otherwise it may
 * wait until the other tasks in this subthread will finish.
 * 
 * @param method
 *          {function} - This is the method, which will be executed once for
 *          every existing subthread.
 * @param param
 *          {object} - Argument for that method.
 * @param doNotRemember
 *          {boolean} - The method definition won't be remembered by future
 *          threads - default false
 */
exports.runOnce = function(method, param, doNotRemember) {
  if (process.subThread) { throw new Error(
          "You can not add a task under a subthread."); }

  exports.begin();
  if (method._taskId) { throw new Error("This method was already defined"); }

  if (param == undefined) {
    param = null;
  }

  var mt = getMethod(method);
  method._taskId = taskId;

  // TODO if null!!! should work
  param = JSON.stringify(param);

  if (doNotRemember == undefined) doNotRemember = false;

  var err = uw.addTask(taskId, mt, param, -2, doNotRemember);
  if (err > 0) { throw new Error("Thread creation error. id:" + err); }

  taskId++;
  trackerId += exports.getThreadCount();

  if (cinter == null) {
    gcc_alive = true;
    runCinter();
  }
};

exports.runOnThread = function(threadId, method, param, cb, obj) {
  if (threadId >= cpuCount) { throw new Error(
          "Given threadId does not exist. You can change the number of threads from setThreadCount"); }

  if (process.subThread) { throw new Error(
          "You can not add a task under a subthread."); }

  exports.begin();

  if (param == undefined) {
    param = null;
  }

  param = JSON.stringify(param);

  var cbId = trackerId;
  if (cb) {
    markers[cbId] = cb;
    objects[cbId] = obj;
  } else {
    cbId = -1;
  }

  var mt = getMethod(method);

  process.sendToThread(threadId, {
    "$runIn": 1,
    "mt": mt,
    "param": param,
    "id": taskId,
    "cbId": cbId
  });
  taskId++;
  trackerId++;

  if (cinter == null) {
    gcc_alive = true;
    runCinter();
  }
};

if (!process.subThread) {
  process.on('threadMessage', function(rs) {
    gcc(rs);
  });
}