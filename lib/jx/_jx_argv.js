...
var singleArg = function(arg, _index) {
  ...
  this.isInt;
  this.asInt;
  this.asBool = true; // default (because the arg exists)
  ...
  this.setValue = function(_value) {
    ...
    // asBool: false in few cases and true otherwise
    var lc = _value.toLowerCase();
    _this.asBool = (lc === "no" || lc === "false" || lc === "0") ? false : true;

    _this.type = typeof this.value;
    _this.hasValue = _this.type !== "undefined";
  };
  ...
  this.splitBySep = function(sep) {
    ...
    var arr = _this.value === undefined ? [] : _this.value.toString().split(sep);
    var ret = [];
    //copying non-empty parts
    for (var a = 0, len = arr.length; a < len; a++)
      if (arr[a].trim()) ret.push(arr[a]);
...
var argvParser = function (arr, options) {
  ...
  while (arg = arr.shift()) {
    ...
    try {
      //var res = path.resolve(arg);
      //var exists = fs.existsSync(res);
    } catch (ex) {
    }
    ...
    // checking for special jx command and eventually mainModule file name in argv
    ...
    if (_arg.prefix) {
      if (options.splitSingleDashed && _arg.prefix === "-" && _arg.name.length > 1) {
