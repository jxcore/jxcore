...
var subscribeOkString = 'Native package has been successfully subscribed to ' +
    'the monitor and runs as a separate process.';
var quitedTooFastString = 'Application did not subscribe to the monitor, ' +
    'because it completes under start_delay interval.';
  ...
  if (cmd == 'run') {
    if (process.argv.length < 4) {
      ...
      if (extName !== '.js' && extName !== '.jx') {
        ...
        if (!isNativePackage) {
          cc.error('The `jx monitor run` command can work only with .js ' +
              'files, .jx packages and native packages.');
