...
var followNativePackage = function() {
  ...
  var fname = path.join(__dirname, "./jxcore." + path.basename(process.mainModule.filename) + ".log");
  ...
  if (cmd == 'run') {
    if (process.argv.length < 4) {
      ...
      if (extName !== ".js" && extName !== ".jx") {
        ...
        if (!isNativePackage) {
          cc.error("The `jx monitor run` command can work only with .js files, .jx packages and native packages.");
          cc.error("The given file is neither of them: " + process.argv[3], "red+bold");
          process.exit(1);
        }
      }
      ...
      if (!isNativePackage) {
        // .js or .jx files
        exports.tryToFollowMeOrExit(function() {
