...
var argNames = {
  ...
  leave_me: "leave_me",
  console: "console",
  respawn_id: "respawn_id" // we can pass respawn id to respawned process by
  // monitor, to track, how many times an app was
  // respawned
};
 ...
 * @return {*}
 */
var parseArgs = function(args, separateInternalArgs) {
  if (!args) { return {}; }
...
var argv = parseArgs(process.argv), appConfig = null, monConfig = null, monLogPath = null, monLogDisabled = false, isWindows = process.platform === 'win32', jxPath = process.argv[0], maxAttemptCount = 10, attemptID = 0, attemptInterval = 1000, monitoredProcesses = {}, massiveKiller = false, // true
// if
// monitor
// shuts
// down
...
// the
// children
tmpLogCache = [], tmpLogCacheFlushed = false, iAmTheMonitor = false; // true,
// if
// current
// process
// is the
...
var writeToLog = function(txt, alsoToConsole) {
  ...
  if (!monLogDisabled) {
    // some logging can occur before config file will be read and log_path will
    // be known
    // so we cache some messages
  ...
  if (fs.existsSync(configFile)) {
    try {
      ...
      if (cfg.monitor.https) {
        // console.log("HTTPS", configFile);
        http = _https;
        https_keys = cfg.monitor.https;
      } else {
        // console.log("HTTP", configFile);
        http = _http;
      }
    } catch (ex) {
...
var subscribe = function(cb) {
  ...
  var env = {
    ...
    okMsg: "Process sucesfully subscribed process " + process.pid
            + " to the monitor."
  };
...
var unsubscribe = function(cb) {
  ...
  var env = {
    ...
    okMsg: "Process sucesfully unsubscribed process " + process.pid
            + " from the monitor."
  };
...
var killPath = function(_path, cb) {
  ...
  var env = {
    ...
    okMsg: "Process sucesfully killed process " + _path + "."
  };
...
var stopChildren = function(cb, arr) {
  ...
  if (!monitoredProcesses) {
    cb(false,
            "There are no children currently monitored, so there is nothing to kill.");
    return;
  }
...
var checkUID = function(cmd, json, res) {
  ...
  if (typeof json.uid === "undefined") {
    ...
    if (isNaN(uid)) {
      ...
      if ((uids && uids[uid]) || process.getuid() === uid || uid === 0) { // force
        // root
        // can
        // close
        return true;
...
var startServer = function() {
  ...
  var __server = function(req, res) {
    ...
    // for example /json?silent=true would not save to log, that request was
    // received
    ...
    if (req_data) {
      ...
      req.on('end', function() {
        ...
        try {
          ...
          var args = parseArgs(json.argv, true);
          ...
          if (cmd == argNames.leave_me && json.pid) {
            ...
            if (ln === 0) {
              ...
              stopChildren(function(err, txt) {
                ...
                res.end(strings.get_dataOK);
                log("Monitor stopped the process with given path");
              }, arr);
              return;
            }
...
var stopOrRestart = function(cb, restart) {
  ...
  isMonitorOnline(function(err, txt) {
    if (!err) {
      ...
      var cmd = argv[argNames.stop_monitor] ? argNames.stop_monitor
...
exports.followMe = function(cb, waitcb) {
  ...
  if (checkForSID_MT(subscribeSID)) {
    if (cb) {
      cb(true,
              "Another subthread aleady is trying to subscribe to the monitor.");
    }
    return;
  }
...
exports.leaveMe = function(cb) {
  ...
  if (checkForSID_MT(unsubscribeSID)) {
    if (cb) {
      cb(true,
              "Another subthread aleady is trying to unsubscribe from the monitor.");
    }
    return;
  }
