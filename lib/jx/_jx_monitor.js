  ...
  for (var i = 0; i < ln; i++) {
    ...
    if (separateInternalArgs) {
      ...
      for (var b in argNames) {
        if (!b || !argNames[b] || !argNames.hasOwnProperty(b)) {
          continue;
        }
...
var sendDataInLoop = function(urlPath, json, expectedAnswer, func, env) {
  ...
  sendRequest(urlPath, json, expectedAnswer, function(err, msg) {
    if (err) {
      var _exit = !iAmTheMonitor && msg.indexOf(strings.command_kill) !== -1;
      if (msg) msg = msg + "";
...
var startServer = function() {
    ...
    if (req_data) {
      ...
      req.on('end', function() {
        ...
        try {
          ...
          var parsedArgv = jxcore.utils.argv.parse(json.argv, { internals : [ argNames.monitor_old_pid ] });
          var previousPid = parsedArgv[argNames.monitor_old_pid];
          if (previousPid) previousPid = previousPid.asInt;
          json.argv = parsedArgv["_"].withoutInternals;
  ...
  process.on("uncaughtException", function(ex) {
    log("Monitor process received uncaughtException: " + ex + ".");
  });
