...
var fs = require('fs'), path = require('path'),
    _http = require('http'), _https = require('https'),
    os = require('os'), util = require('util'), _url = require('url');
...
var parsedArgv = jxcore.utils.argv.parse(),
    ...
    massiveKiller = false, tmpLogCache = [], tmpLogCacheFlushed = false,
        ...
        iAmTheMonitor = false, exitting = false, monitorUrl = '',
 ...
 * @param {string} txt
 *          - text to log.
 * @param {boolean} alsoToConsole
 *          - is used in some critical point to write msg not only to log,
 *          but also to console.
 */
var monCounter = 0;
 ...
 *
 * @param {string} path
 *          - this is the last part of the url (http://host:port/path).
 * @param {object} jsonData
 *          - this object (stringified) will be sent by POST method.
 * @param {string} expectedAnswer
 *          - The answer from monitor is compared with this value. If
 *          they are equal, sendRequest is considered to be successful.
 * @param {function} cb
 *          - this callback will be invoked after sendRequest
      ...
      if (cb) {
        ...
        if (body.toString().indexOf(expectedAnswer) === -1) {
          ...
          var err = refused ? bodyStr :
              'Problem with connecting to the monitor. ' +
              'Received not what expected: ' + bodyStr;
          cb(true, err);
        } else {
          cb(false, bodyStr);
        }
 ...
 *
 * @param {function} cb
 *          - this callback will receive 3 params
 *          (online true/false, err, txt).
 */
var isMonitorOnline = function(cb) {
  ...
  // might be online
  sendRequest(strings.send_question, null, strings.get_answer,
              function(err, txt) {
        if (err) {
          cb(true, txt);
        } else {
 ...
 *
 * @param {string} configFile - full path.
 ...
 *
 * @param {object} json_from - object to copy fields from.
 * @param {object} json_to - object to copy fields to.
 * @param {string} prefix - internal use. Should be empty on first call.
 */
 ...
 *          they are equal, sendRequest is considered to be successful.
 * @param {function} func
 *          - this function should be called on next attempt.
 * @param {object} env
 *          - this object should have 3 members { cb, errMsg, okMsg }.
  ...
  sendRequest(
      ...
      function(err, msg) {
        if (err) {
          ...
          if (msg)
            msg = msg + '';
          if (msg && msg.indexOf(strings.command_refuse) > -1 &&
              env && env.cb) {
            ...
            if (_exit) {
              s = 'Monitor says, that the app was already killed and needs ' +
                  'to be terminated. Exiting now.';
...
var subscribe = function(cb) {
  ...
  if (process.isPackaged) {
    ...
    var fst = process.argv[1].toLowerCase(), scd = (len > 2) ?
        process.argv[2].toLowerCase() : '';

    if (fst == 'monitor' && scd == 'run' && len > 3) {
      _path = path.resolve(process.argv[3]);
    } else if (fst == 'mt' || fst == 'mt-keep' ||
        fst.indexOf('mt-keep:') >= 0 ||
        fst.indexOf('mt:') >= 0) {
 ...
 *
 * @param {object} json
 *          - This is an object sent from the client. It must have a
 *          json.pid field.
 */
 ...
 * @param {array} arr - list of monitored children or null to kill all.
 * @param {object} env - object to report results.
 */
var stopChildren = function(cb, arr, env) {
  ...
  if (!monitoredProcesses) {
    env.msg = 'There are no children currently monitored, so there is ' +
        'nothing to kill.';
    cb(env);
    return;
  }
  ...
  for (var pid in _arr) {
    ...
    if (_arr[pid].killAttempt < maxAttemptCount) {
      var pid_to_kill = _arr[pid].respawning && _arr[pid].child ?
          _arr[pid].child.pid : pid;
      var isAlive = false;
  ...
  if (env.total != env.killed.length + env.killFailed.length) {
    ...
    if (env.total == 0) {
      env.report.push('There are no children currently monitored, ' +
 ...
 * @param {object} res - http result object where to send response in case of
 *         error.
  ...
  if (typeof json.uid === 'undefined') {
    ...
    if (isNaN(uid)) {
      ...
      if ((uids && uids[uid]) || process.getuid() === uid || uid === 0) {
        ...
        error = 'Refused to run the command since it is made by a ' +
            'different user.';
      }
    }
  }
              ...
              function() {
                ...
                try {
                  ...
                  if (!monitoredProcesses[json.pid]) {
                    if (json.old_pid && (killedProcesses[json.old_pid] ||
                        !monitoredProcesses[json.old_pid])) {
    ...
    if (req_logs) {
      if (fs.existsSync(monLogPath)) {
        try {
          ...
          ret = 'Cannot read log file from ' + monLogPath +
 ...
 *          - If true, the current process will create new http server
 ...
 *
 * @param {function} cb
 *          - Callback will receive 2 params (err true/false, txt).
 */
 ...
 *
 * @param {function} cb
 *          - Callback is optional. When provided will receive 2
...
exports.leaveMe = function(cb) {
  ...
  if (checkForSID_MT(unsubscribeSID)) {
    if (cb) {
      cb(true, 'Another subthread already is trying to unsubscribe ' +
          'from the monitor.');
    }
    return;
  }
 ...
 * server for that
 *
 * @param {object} monConfig - configuration.
 * @return {object} - remote access configuration.
 */
var initRemoteAccess = function(monConfig) {
  ...
  if (ra.port === cfg.port)
    return _logError('This feature needs to use separate port ' +
        'than the monitor.');
 ...
 * @return {*} - Returns error string or loaded certificate files (if
 *         everything is ok).
 */
