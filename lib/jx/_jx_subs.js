...
process.release = function(customCounter){
  ...
  if(!restarted && !process.__reset){
    setTimeout(function(){
      ...
      if(num==0){return;}
        tw.sendToAll(-1, JSON.stringify({threadId:process.threadId, params:null, wait:-1 * num}), process.threadId);
...
var cnt = 0;
var cntMax = 3000;
var triggerEnabled = true, cnter = 0;
var taskChecker;
var inter;
var cached = {};
...
var runner = function(param){
	...
	if(cached[name]){
		...
		thread = require(name, 0,0,3, ex_data.readMap(process.threadId, "*" + name));
		cached[name] = thread;
	}
	var w = JSON.parse(param[2]);
	try{
		if(!thread.call){
...
if(MTnoKeep){_timeout = setTimeout;}
...
taskChecker.toString = function(){return null;};
