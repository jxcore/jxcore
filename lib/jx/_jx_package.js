...
var normalizeExtractOptions = function(ret, isNative) {
  ...
  if (ret && isNative && !isAppRoot) {
    jxcore.utils.console
        .error("Native packages `--extract` option is supported only with `--extract-app-root` added.");
  ...
  if (isPartialExtract) {
    if (!isAppRoot) {
      jxcore.utils.console
          .error("Partial extract (`--extract-what`) is supported only with `--extract-app-root` added.");
      jxcore.utils.console.warn(strWhereDir);
      process.exit(1);
    }
...
exports.compile = function(argv, ops) {
  ...
  if (warn_node) {
    ...
    jxcore.utils.console
        .info("However you may mark .node files as extractable, e.g.: --extract-what '*.node'");
  }
