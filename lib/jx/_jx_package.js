...
exports.getPatchMatcher = function(parms) {
  ...
  ret.test = function(full_path) {
    var basename = path.basename(full_path);
    return ret[basename] === 1 || ret[full_path] === 2 ||
      ret.isWildcardMatching(basename, full_path);
  };
...
var getPackageName = function(argv) {
  ...
  if (!name || name.slice(0, 1) === "-") {
    ...
    var parsedArgv = jxcore.utils.argv.parse();
    if (parsedArgv.name && parsedArgv.name.hasValue && parsedArgv.name.value.length)
...
var getExtractOptions = function() {
  ...
  if (parsedArgv["extract-what"]) {
    if (typeof ret === "boolean")
      ret = {};
    ret["what"] = parsedArgv["extract-what"].splitBySep();
  }
...
var normalizeExtractOptions = function(ret, isNative) {
  ...
  var strWhereDir = "This is because the content should be extracted into package's location folder rather than a sub directory.";
  var isExtract = !!ret;
  var isPartialExtract = isExtract && ret["what"];
  var isAppRoot = ret["where"] === "./";

  if (ret && isNative && !isAppRoot) {
    jxcore.utils.console.error("Native packages `--extract` option is supported only with `--extract-app-root` added.");
  ...
  if (isPartialExtract) {
    if (!isAppRoot) {
      jxcore.utils.console.error("Partial extract (`--extract-what`) is supported only with `--extract-app-root` added.");
      jxcore.utils.console.warn(strWhereDir);
      process.exit(1);
    }
...
exports.compile = function(argv, ops) {
  ...
  if (extractOptions && extractOptions["what"]) {
    ...
    if (util.isArray(extract_what))
      extract_what = require("_jx_package").getPatchMatcher(extractOptions["what"]);
  }
  ...
  if (proj.assets) {
    ...
    for (var i = 0; i < ln; i++) {
      ...
      if (path.extname(loc) == '.node') {
        var matches = extractOptions && (!extract_what || (extract_what && extract_what.test && extract_what.test(loc)));
        if (!matches)
          warn_node = loc;
      }
  ...
  if (warn_node) {
    ...
    jxcore.utils.console.info("However you may mark .node files as extractable, e.g.: --extract-what '*.node'");
  }
