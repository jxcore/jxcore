...
var binaryFileIndexOf = function (file_name, testString, writeDotAt) {
  ...
  while (true) {
    ...
    // this is much faster for SM (and V8 too) than iterating through buffer chars!
    ...
    if (id !== -1) {
      ...
      // so there is no performance penalty here, if the searched string is unique
...
exports.compile = function (argv, ops) {
  ...
  if (contents.project.native) {
      ...
      ? "copy /Y" : "cp", copy_ext = os_info.isWindows ? ".exe" : "";
    {
      ...
      // first string
      var buffer = new Buffer(parseInt((5 * cmped.length) - 123456789) + "").toString('hex');
      buffer += ")";
