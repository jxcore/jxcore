  ...
  if (contents.project.native) {
    ...
    if (os_info.isWindows) {
      ...
      if (!verpatch || error) {
        cc("");
        if (!verpatch)
          cc("Cannot apply file description information. The '.ver' file not found.",
              "magenta");
        if (error)
...
exports.package = function(argv) {
  ...
  try {
    ...
    var addOnlyStartupFile = parsedArgv.add && (!parsedArgv.add.hasValue || parsedArgv.add.asBool);
