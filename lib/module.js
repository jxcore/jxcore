...
Module._oldRes = function (request, parent) {
  ...
  if (!filename && (process.subThread || process.IsEmbedded) && parent && parent.parent && parent.parent.filename) {
...
Module._resolveFilename = function (r, p, ___, ____, __subCheck) {
  ...
  // there is nothing else with the same name
  // check if it was requiring back to itself
  if (found_target) {
