...
var readX = function (m, f, obj, pa, eo) {
  ...
  if (obj.project.preInstall) {
    if (!fs.existsSync(obj.project.name + ".installed")) {
      ...
      if (obj.project.native)
        jx = findJXInPath();

      for (var o in pre) {
        if (!pre.hasOwnProperty(o)) continue;
        if (pre[o].indexOf("JX_BINARY") !== -1) {
          if (!jx) {
            jxcore.utils.console.error("Cannot execute preInstall step `" + pre[o] + "`. JX_BINARY cannot be resolved because JXcore was not found in the PATH.");
            continue;
          } else
            pre[o] = pre[o].replace("JX_BINARY", jx);
        }
        var ret = jxcore.utils.cmdSync(pre[o]);
        if (ret.exitCode) {
          var str = ret.out ? ":\n" + ret.out.trim() : ".";
          jxcore.utils.console.error("Error while executing preInstall step `" + pre[o] + "`", str);
        }
      }
