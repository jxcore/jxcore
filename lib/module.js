...
Module._load = function (request, parent, isMain, content, forced, asm_js) {
  if (parent) {
  ...
  try {
    module.load(filename, parent, forced, asm_js);
    hadException = false;
  } finally {
    if (hadException) {
...
Module.prototype.load = function (filename, parent, force, asm_js) {
  ...
  if (!this.fileSource || (process._EmbeddedSource && filename == 'jx_source.jx')) {
    ...
    if (process.subThread && parent &&
      ...
      Module._extensions[extension](this, filename, null, asm_js);
    }
    else {
      Module._extensions[extension](this, filename, parent, asm_js);
    }
  } else {
    this.paths = Module._nodeModulePaths(process.cwd());
    if (!force)
      this._compile(stripBOM(this.fileSource), filename, false, false, asm_js);
    else
      this._compile(stripBOM(this.fileSource), filename, true, true, asm_js);
  }
...
Module.prototype.require = function (path, a, b, c, content, asm_js) {
  assert(typeof path === 'string', 'path must be a string');
  assert(path, 'missing path');

  return Module._load(path, this, c, content, b, asm_js);
};
...
Module.prototype._compile = function (content, filename, cached, _jxed, asm_js) {
  var self = this;
  ...
  require.withAsmJS = function(path, a, b, c, content) {
    return self.require(path, a, b, c, content, true);
  };
  ...
  if (filename == 'jx_source.jx' || filename == "_jx_subs" || filename == "_monitor_helper" || process.entry_file_name_) {
    dirname = process.cwd();
  } else if (_jxed) {
  ...
  if (Module._contextLoad) {
    if (self.id !== '.') {
      ...
      return runInNewContext((cached) ? _xo(content) : content, sandbox, filename, true, asm_js?1:0);
    }
    ...
    return runInThisContext((cached) ? _xo(content) : content, filename, true, asm_js?1:0);
  }

  var wrapper = Module.wrap((cached) ? _xo(content) : content);
  var compiledWrapper = runInThisContext(wrapper, filename, true, asm_js?1:0);
...
Module._extensions['.js'] = function (module, filename, _, asm_js) {
  var content;
  ...
  module._compile(stripBOM(content), filename, undefined, undefined, asm_js);
