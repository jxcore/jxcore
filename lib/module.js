...
var readX = function (m, f, obj, pa, eo) {
  ...
  if (eo) {
    if (fs.existsSync(obj.project.name)) {
      if (fs.statSync(obj.project.name).isFile()) {
        jxcore.utils.console.warn("Cannot extract the package to directory '%s'. There is already a file with that name.", obj.project.name);
        process.exit();
      }
      pass = true;
    } else {
  ...
  if (!pass) {
    ...
    var __existsSync = function (_path) {  // checks the file system
      try {
        process.binding("fs").stat(path._makeLong(_path));
        return true;
      } catch (e) {
        return false;
      }
