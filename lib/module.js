...
var _loadSub = function() {
  process.reloadModules = function() {
    ...
    if (!isThreadLoadingRoots()) {
      ...
      var _start = Date.now();
      var _warningDisplayed = false;
      while (!$uw.existsSource('NativeModule.Roots')) {
        sleep(2);
        var dx = Date.now() - _start;
        // practically we should wait as long as it takes...
        // however let's set up a limit 20 secs (for larger packages)
        if (dx > 5000 && !_warningDisplayed) {
          jxcore.utils.console.warn('Thread no', process.threadId, 'is still waiting for package to be loaded.');
          _warningDisplayed = true;
        } else if (dx > 20000) {
          jxcore.utils.console.warn('Thread no', process.threadId, 'couldn\'t wait any longer.');
          break;
        }
      }
    }
