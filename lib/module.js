...
var findJXInPath = function() {
  ...
  var ret = jxcore.utils.cmdSync(process.platform === "win32" ? "where jx" : "which jx" );
  ...
  var ret = jxcore.utils.cmdSync(_path + ' -p "JSON.stringify(process.versions)"');
  try {
...
Module._oldRes = function (request, parent) {
  ...
  if (!filename && (process.subThread || process.isPackaged) && parent && parent.parent && parent.parent.filename) {
...
var runActions = function(actions, argName, native, verbose) {
  ...
  for(var o = 0, len = actions.length; o < len; o++) {
    ...
    if (cmd.indexOf("JX_BINARY") !== -1) {
      if (!jx) {
        var str = "JX_BINARY cannot be resolved because JXcore was not found in the PATH.";
        if (verbose)
          jxcore.utils.console.error(" Error:\n" + str);
        else // if not verbose, show error anyway
          jxcore.utils.console.error("Cannot execute", quotedArg, "step", quotedCmd, ":\n" + str);
    ...
    if (ret.exitCode) {
      ...
      if (verbose)
        ...
        jxcore.utils.console.error("Error while executing", quotedArg, "step", quotedCmd, ":\n" + str);
      err = true;
    }
...
var readX = function (m, f, obj, pa, eo) {
  ...
  if (!obj.project.library && !entry_file && process.argv[1] !== "monitor" && process.argv[2] !== "run") {
  ...
  var extract_verbose = eo ? jxcore.utils.argv.isBoolValue(eo["verbose"]).isTrue : false;
  var extract_overwrite = eo ? jxcore.utils.argv.isBoolValue(eo["overwrite"]).isTrue : false;
  ...
  if (eo && subDir) {
    if (fs.existsSync(subDir)) {
      ...
      if (stat.isFile()) {
        jxcore.utils.console.warn("Cannot extract the package to directory '%s'. There is already a file with that name.", subDir);
        process.exit();
      } else if (stat.isDirectory() && !extract_overwrite) {
        jxcore.utils.console.warn("Cannot extract the package to directory '%s'. There is already a folder with that name.\n" +
        "Use `--extract-overwrite` switch for `jx package` command.", subDir);
        process.exit();
      }
      pass = true;
    } else {
  ...
  if (!pass) {
    ...
    if (eo["pre-actions"])
      runActions(eo["pre-actions"], "extract-pre-actions", obj.project.native, extract_verbose);
    ...
    for (var o in obj.docs) {
      ...
      if (doExtract) {
        ...
        if (extract_verbose) {
          ...
          if (write)
            jxcore.utils.console.log(jxcore.utils.console.setColor("extracting", "yellow"), a2);
          else
            jxcore.utils.console.log(jxcore.utils.console.setColor("skipping (file already exists)", "yellow"), a2);
        }
    ...
    // one-time adding folders not containing any files, but containing sub-folders
    ...
    if (eo["post-actions"])
      runActions(eo["post-actions"], "extract-post-actions", obj.project.native, extract_verbose);
  }
...
Module._resolveFilename = function (r, p, ___, ____, __subCheck) {
  ...
  if (!found_target && res1 === 0) found_target = a; // do not use this yet. search for modules
  ...
  if (!found_target && res === 0) found_target = a; // do not use this yet. search for modules
  if (res && !isModuleName) {
    return res;
  }
