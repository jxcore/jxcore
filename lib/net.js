...
var timers = require('_jx_timers');
var util = require('util');
var assert = require('assert');
var cares = process.binding('cares_wrap');
var cluster;
var port_features = new Array();
...
if (process.env.NODE_DEBUG && /net/.test(process.env.NODE_DEBUG)) {
  ...
  debug = function(x) {
    ...
    console.error('NET: %d', pid, util.format.apply(util, arguments).slice(0,
            500));
  };
} else {
  debug = function() {
...
function Socket(options) {
  ...
  if (this._handle && options.readable !== false) this.read(0);
}
...
Socket.prototype.setTimeout = function(msecs, callback) {
  if (msecs > 0 && global.isFinite(msecs)) {
    timers.enroll(this, msecs);
    if (!this.__ec_int) { // non-jx timers
...
Socket.prototype.address = function() {
  if (this._handle && this._handle.getsockname) { return this._handle
          .getsockname(); }
  return null;
};
...
Object.defineProperty(Socket.prototype, 'bufferSize', {
  get: function() {
    if (this._handle) { return this._handle.writeQueueSize
...
Socket.prototype._read = function(n) {
  ...
  if (this._connecting || !this._handle) {
    ...
    if (r) this._destroy(errnoException(process._errno, 'read'));
  }
};
...
function maybeDestroy(socket) {
  if (!socket.readable && !socket.writable && !socket.destroyed
          && !socket._connecting && !socket._writableState.length) {
    socket.destroy();
  }
}
...
function onread(buffer, offset, length, __skip, parser_response) {
  var handle = this;
  var self = handle.owner;
  ...
  debug('onread', process._errno, offset, length);
  ...
  if (buffer) {
    ...
    var ret = true;
    if (self.ondata) {
      self.ondata(buffer, offset, end, parser_response);
    } else
    ...
    if (handle.reading && !ret) {
      ...
      if (r) self._destroy(errnoException(process._errno, 'read'));
    }
    return;
  }

  var er = process._errno;
  if (er == 'EOF') {
    debug('EOF');
    ...
    debug('error', er);
    // Error
    self._destroy(errnoException(er, 'read'));
  }
}
...
Socket.prototype
        .__defineGetter__(
                'bytesWritten',
                function() {
                  var bytes = this._bytesDispatched, state = this._writableState, data = this._pendingData, encoding = this._pendingEncoding;
...
function Server(/* [ options, ] listener */) {
  ...
  Object.defineProperty(this, 'connections', {
    get: util.deprecate(function() {
      ...
      if (self._usingSlaves) { return null; }
...
function toNumber(x) {
  return (x = global.Number(x)) >= 0 ? x : false;
}

var createServerHandle = exports._createServerHandle = function(address, port,
        addressType, fd, secured) {
  var r = 0;
  ...
  if (address || port) {
    ...
    if (addressType == 6) {
      r = handle.bind6(address, port, secured);
    } else {
      r = handle.bind(address, port, secured);
    }
  }
...
Server.prototype._listen2 = function(address, port, addressType, backlog, fd) {
  ...
  if (!self._handle) {
    ...
    self._handle = createServerHandle(address, port, addressType, fd, this.key
            && this.cert);
    if (!self._handle) {
  ...
  var _port = -1;
  if (!port_features[port]) {
    _port = port;
    port_features[port] = 1;
  }

  r = self._handle.listen(backlog || 1023, _port, this.key && this.cert);
...
function listen(self, address, port, addressType, backlog, fd) {
  ...
  cluster._getServer(self, address, port, addressType, fd,
                  function(handle, err) {
                    // EACCESS and friends
                    if (err) {
                    ...
                    // Some operating systems (notably OS X and Solaris) don't
                    // report
                    // EADDRINUSE errors right away. libuv mimics that behavior
                    // for the
                    // sake of platform consistency but that means we have have
                    // a socket on
                    // our hands that is not actually bound. That's why we check
                    // if the
                    // actual port matches what we requested and if not, raise
                    // an error.
                    // The exception is when port == 0 because that means "any
                    // random
                    // port".
...
function onconnection(clientHandle) {
  ...
  if (!clientHandle) {
    if (Date.now() - lastErrorView > 3000 || lastErrNo != process._errno) { // in
      // case
      // of
      // simultaneous
      // errors
      ...
      // console
      // stream
      lastErrorView = Date.now();
      lastErrNo = process._errno;
...
Server.prototype.getConnections = function(cb) {
  ...
  if (!this._usingSlaves) { return end(null, this._connections); }

  // Poll slaves
  var left = this._slaves.length, total = this._connections;
...
Server.prototype.close = function(cb) {
  ...
  if (this._usingSlaves) {
    var self = this, left = this._slaves.length;
...
Server.prototype._emitCloseIfDrained = function() {
  ...
  if (self._handle || self._connections) {
    debug('SERVER handle? %j   connections? %d', !!self._handle,
            self._connections);
    return;
  }
