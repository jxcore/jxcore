...
function checkServerIdentity(host, cert) {
  ...
  function regexpify(host, wildcards) {
    ...
    if (!wildcards && /\*/.test(host) || /[\.\*].*\*/.test(host)
            || /\*/.test(host) && !/\*.*\..+\..+/.test(host)) { return /$./; }

    // Replace wildcard chars with regexp's wildcard and
    // escape all characters that have special meaning in regexps
    // (i.e. '.', '[', '{', '*', and others)
    var re = host.replace(/\*([a-z0-9\\-_\.])|[\.,\-\\\^\$+?*\[\]\(\):!\|{}]/g,
            function(all, sub) {
...
CryptoStream.prototype._writePending = function writePending() {
  var data = this._pending, encoding = this._pendingEncoding, cb = this._pendingCallback;
...
CryptoStream.prototype._read = function read(size) {
  ...
  var bytesRead = 0, start = this._buffer.offset, last = start;
  do {
...
CryptoStream.prototype.getSession = function() {
  if (this.pair.ssl) { return this.pair.ssl.getSession(); }

  return null;
};

CryptoStream.prototype.isSessionReused = function() {
  if (this.pair.ssl) { return this.pair.ssl.isSessionReused(); }
...
CryptoStream.prototype._done = function() {
  ...
  if (this.pair.cleartext._doneFlag && this.pair.encrypted._doneFlag
          && !this.pair._doneFlag) {
...
function onclienthello(hello) {
  ...
  if (hello.sessionId.length <= 0 || !this.server
...
function SecurePair(credentials, isServer, requestCert, rejectUnauthorized,
        options) {
  if (!(this instanceof SecurePair)) { return new SecurePair(credentials,
          isServer, requestCert, rejectUnauthorized, options); }
  ...
  this.ssl = new Connection(this.credentials.context, this._isServer ? true
          : false, this._isServer ? this._requestCert : options.servername,
...
exports.createSecurePair = function(credentials, isServer, requestCert,
        rejectUnauthorized) {
  var pair = new SecurePair(credentials, isServer, requestCert,
          rejectUnauthorized);
  return pair;
};
...
SecurePair.prototype.error = function(returnOnly) {
  ...
  } else if (this._isServer && this._rejectUnauthorized
...
function Server(/* [options], listener */) {
  ...
  if (!self.pfx && (!self.cert || !self.key)) { throw new Error(
  ...
  if (typeof timeout !== 'number') { throw new TypeError(
          'handshakeTimeout must be a number'); }
  ...
  net.Server.call(this, function(socket) {
    ...
    var pair = new SecurePair(creds, true, self.requestCert,
...
Server.prototype.setOptions = function(options) {
  ...
  if (options.sessionIdContext) {
    ...
    this.sessionIdContext = crypto.createHash('md5').update(
            process.argv.join(' ')).digest('hex');
  }
...
Server.prototype.addContext = function(servername, credentials) {
  if (!servername) { throw 'Servername is required parameter for Server.addContext'; }
...
exports.connect = function(/* [port, host], options, cb */) {
  ...
  var hostname = options.servername || options.host || 'localhost', pair = new SecurePair(
          sslcontext, false, true, options.rejectUnauthorized === true ? true
                  : false, {
  ...
  if (!options.socket) {
    var connect_opt = (options.path && !options.port) ? {
      path: options.path
    } : {
      port: options.port,
      host: options.host,
  ...
  pair.on('secure', function() {
    ...
    if (!verifyError) {
      var validCert = checkServerIdentity(hostname, pair.cleartext
              .getPeerCertificate());
      if (!validCert) {
