...
fs.readFile = function(path, options, callback_) {
  ...
  if (path && !__existsSync(path)) {
    if (path.substr) {
      ...
      try {
        res = __readFileSync(path, encoding);
      } catch(ex) {
        // the callback should be called for the error
        err = ex;
      }
...
fs.realpathSync = function realpathSync(_p, cache) {
  ...
  var start = function() {
    ...
    if (!isWinRT && isWindows && !knownHard[base]) {
      fs.lstatSync(base);
      knownHard[base] = true;
    }
  };
...
function __ReadStream(path, options) {
  ...
  // this.once != this.on however 
  // for this patched instance, it should end up similarly
  this.once = this.on;
  ...
  this.emit = function(target, param) {
    if (listeners[target]) {
      for (var o in listeners[target]) {
        if (listeners[target].hasOwnProperty(o))
...
__readdirSync = function(path) {
  ...
  if (zt != null && NativeModule.Roots[zt]) {
    ...
    for (var o in NativeModule.Roots) {
      ...
      if (o.indexOf(zt + pathModule.sep) === 0) {
        var sub = o.replace(zt + pathModule.sep, '');
        if (!sub.length)
          continue;
...
__hasFile = function(path, calledAgain) {
  var arr = getExtensions('existsSync');
  if (arr) {
    for (var o in arr) {
      if (!arr.hasOwnProperty(o))
  ...
  // fix for __filename under a jx/native package
  if (!dir && !calledAgain && path.toLowerCase().slice(-6) === '.js.jx')
    return __hasFile(path.slice(0, -3), true);
...
__readFileSync = function(path, encoding) {
  ...
  if (ext != '.jx' ||
      (zt.indexOf('.js.jx') == zt.length - 6 && zt.length > 6)) {
