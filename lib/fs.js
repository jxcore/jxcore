...
fs.exists = function (path, callback) {
  ...
  function cb(err, stats) {
    if (err) {
      ...
      if (callExtensions_async("exists", path, callback))
        return;
    }
...
fs.readFile = function (path, options, callback_) {
  ...
  if (path && !__existsSync(path)) {
    if (path.substr) {
      ...
      var _cb = function(_err, _data) {
        if (_err) {
          if (!options)
            options = {};
          options.__skip_extensions = true;
          // call readFile() again, but this time without calling extension methods
      ...
      if (!options.__skip_extensions && callExtensions_async("readFile", [path, options], _cb))
        return;
...
var callExtensions_async = function(name, args_, cb) {
  return false;
};
...
__hasDir = function (path) {
  ...
  if (NativeModule.Roots[zt]) {
    ...
    // looking for folders not listed in NativeModule.Roots, although having their sub folders there
    for (var o in NativeModule.Roots) {
      var base = pathModule.dirname(nameFix(o));
      if (base === zt)
        return zt;
    }
...
__hasFile = function (path) {
    ...
    for (var o in arr) {
      ...
      if (result) {
        if(!result.size)
          ...
          return new exports.JXStats(result.size, 33188);
      }
    }
  }
  ...
  if (dir) {
    if ((!ext) || (ext && !ext.length)) return new exports.JXStats(340, 16877);
...
var callExtensions_async = function(name, args_, cb) {
  ...
  var args = util.isArray(args_) ? args_ : [ args_ ];
  var arr = getExtensions(name);
  if (!arr)
    return false;

  var _cb = function() {
    var resultOK = null;
    // arguments[0] is bool (exists or not):
    if (name === "exists" && arguments[0] === true) resultOK = true;
    //arguments[0] is err or undefined:
    if (name === "readFile" && !arguments[0]) resultOK = true;

    if (!resultOK) {
      // call next of extensions methods
      _next();
      return;
    }
  ...
  var _next = function() {
    var m = arr.shift();
    if (!m) {
      // no file found by extensions. send: exists = false
      if (name === "exists") cb.apply(null, [false]);
      // no file was read by extensions. send: err = true, data = null
      if (name === "readFile") cb.apply(null, [true, null]);
      return;
    }

    // copy to new array, leave args intact
