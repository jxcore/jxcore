 ...
 * 
 * var repl = require("repl"); // start repl on stdin repl.start("prompt> ");
 *  // listen for unix socket connections and start repl on them
 * net.createServer(function(socket) { repl.start("node via Unix socket> ",
 * socket); }).listen("/tmp/node-repl-sock");
 *  // listen for TCP socket connections and start repl on them
 * net.createServer(function(socket) { repl.start("node via TCP socket> ",
 * socket); }).listen(5001);
 *  // expose foo to repl context repl.start("node > ").context.foo = "stdin is
 * fun";
 */
...
function REPLServer(prompt, stream, eval_, useGlobal, ignoreUndefined) {
  if (!(this instanceof REPLServer)) { return new REPLServer(prompt, stream,
          eval_, useGlobal, ignoreUndefined); }
  ...
  if (match) {
    ...
    if (line.length === 0 || match) {
      ...
      if (!expr) {
        ...
        if (this.useGlobal || this.context.constructor
 ...
 * 
 * @param {keyword}
 *          keyword The command entered to check.
...
REPLServer.prototype.defineCommand = function(keyword, cmd) {
  ...
  } else if (typeof cmd.action !== 'function') { throw new Error(
          'bad argument, action must be a function'); }
...
function addStandardGlobals(completionGroups, filter) {
  ...
  completionGroups.push(['NaN', 'Infinity', 'undefined', 'eval', 'parseInt',
      'parseFloat', 'isNaN', 'isFinite', 'decodeURI', 'decodeURIComponent',
      'encodeURI', 'encodeURIComponent', 'Object', 'Function', 'Array',
  ...
  if (filter) {
    completionGroups.push(['break', 'case', 'catch', 'const', 'continue',
        'debugger', 'default', 'delete', 'do', 'else', 'export', 'false',
        'finally', 'for', 'function', 'if', 'import', 'in', 'instanceof',
        'let', 'new', 'null', 'return', 'switch', 'this', 'throw', 'true',
...
function trimWhitespace(cmd) {
  var trimmer = /^\s*(.+)\s*$/m, matches = trimmer.exec(cmd);
 ...
 * 
 * @param {String}
 *          cmd The cmd to convert.
 * @return {String} The converted command.
 */
REPLServer.prototype.convertToContext = function(cmd) {
  var self = this, matches, scopeVar = /^\s*var\s*([_\w\$]+)(.*)$/m, scopeFunc = /^\s*function\s*([_\w\$]+)/;

  // Replaces: var foo = "bar"; with: self.context.foo = bar;
  matches = scopeVar.exec(cmd);
  if (matches && matches.length === 3) { return 'self.context.' + matches[1]
          + matches[2]; }
...
/**
 * Returns `true` if "e" is a SyntaxError, `false` otherwise. This function
 * filters out false positives likes JSON.parse() errors and RegExp syntax
 * errors.
 */
function isSyntaxError(e) {
  ...
  return e
          ...
          && !e
                  .match(/^SyntaxError: Invalid flags supplied to RegExp constructor/)
          &&
