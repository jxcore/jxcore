  ...
  self._handle.lookup(address, function(err, ip) {
    ...
    if (cluster.isWorker) {
      cluster._getServer(self, ip, port, self.type, -1, function(handle, err) {
        if (err) return self.emit('error', errnoException(err, 'bind'));
...
Socket.prototype.send = function(buffer, offset, length, port, address,
  ...
  offset = offset | 0;
  if (offset < 0) throw new RangeError('Offset should be >= 0');
  ...
  length = length | 0;
  if (length < 0) throw new RangeError('Length should be >= 0');
...
function afterSend(status, handle, req, buffer) {
  ...
  if (req.cb) req.cb(null, buffer.length); // compatibility with
                                            // dgram_legacy.js
}
...
Socket.prototype.address = function() {
  ...
  if (!address) throw errnoException(process._errno, 'getsockname');

  return address;
};

Socket.prototype.setBroadcast = function(arg) {
  if (this._handle.setBroadcast((arg) ? 1 : 0)) { throw errnoException(
          process._errno, 'setBroadcast'); }
};
...
Socket.prototype.setTTL = function(arg) {
  ...
  if (this._handle.setTTL(arg)) { throw errnoException(process._errno, 'setTTL'); }
...
Socket.prototype.setMulticastTTL = function(arg) {
  ...
  if (this._handle.setMulticastTTL(arg)) { throw errnoException(process._errno,
          'setMulticastTTL'); }
...
Socket.prototype.setMulticastLoopback = function(arg) {
  ...
  if (this._handle.setMulticastLoopback(arg)) { throw errnoException(
          process._errno, 'setMulticastLoopback'); }
...
Socket.prototype.addMembership = function(multicastAddress, interfaceAddress) {
  ...
  if (!multicastAddress) { throw new Error(
          'multicast address must be specified'); }

  if (this._handle.addMembership(multicastAddress, interfaceAddress)) { throw new errnoException(
          process._errno, 'addMembership'); }
};
...
Socket.prototype.dropMembership = function(multicastAddress, interfaceAddress) {
  ...
  if (this._handle.dropMembership(multicastAddress, interfaceAddress)) { throw new errnoException(
          process._errno, 'dropMembership'); }
};

Socket.prototype._healthCheck = function() {
  if (!this._handle) throw new Error('Not running'); // error message from
                                                      // dgram_legacy.js
};
...
function onMessage(handle, slab, start, len, rinfo) {
  var self = handle.owner;
  if (!slab) { return self.emit('error', errnoException(process._errno,
          'recvmsg')); }
