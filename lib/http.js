...
OutgoingMessage.prototype._writeRaw = function(data, encoding) {
  ...
  if (this.connection && this.connection._httpMessage === this
          && this.connection.writable && !this.connection.destroyed) {
  ...
  if (state.sentConnectionHeader === false) {
    var shouldSendKeepAlive = this.shouldKeepAlive
            && (state.sentContentLengthHeader
                    || this.useChunkedEncodingByDefault || this.agent);
...
OutgoingMessage.prototype.getHeader = function(name) {
  if (arguments.length < 1) { throw new Error(
          '`name` is required for getHeader().'); }
...
OutgoingMessage.prototype.removeHeader = function(name) {
  if (arguments.length < 1) { throw new Error(
          '`name` is required for removeHeader().'); }

  if (this._header) { throw new Error(
...
function chunkify(chunk, headers, trailers, last) {
  ...
  switch (octets) {
  case 8:
    buf[offset++] = hex((chunklen >>> 28) & 15);
  case 7:
    buf[offset++] = hex((chunklen >>> 24) & 15);
  case 6:
    buf[offset++] = hex((chunklen >>> 20) & 15);
  case 5:
    buf[offset++] = hex((chunklen >>> 16) & 15);
  case 4:
    buf[offset++] = hex((chunklen >>> 12) & 15);
  case 3:
    buf[offset++] = hex((chunklen >>> 8) & 15);
  case 2:
    buf[offset++] = hex((chunklen >>> 4) & 15);
  }
...
OutgoingMessage.prototype.write = function(chunk, encoding) {
  ...
  if (typeof chunk !== 'string' && !Buffer.isBuffer(chunk)) { throw new TypeError(
  ...
  if (this.chunkedEncoding) {
    if (typeof (chunk) === 'string' && encoding !== 'hex'
            && encoding !== 'base64') {
...
OutgoingMessage.prototype.end = function(data, encoding) {
  ...
  var hot = this._headerSent === false && ln && this.output.length === 0
          && this.connection && this.connection.writable
  ...
  if (hot) {
    if (!lnbuffer && typeof data === 'string') {
      if (this.chunkedEncoding) {
        ...
        ret = this.connection.write(this._header + l + CRLF + data
                + '\r\n0\r\n' + this._trailer + '\r\n', encoding);
      } else {
...
ServerResponse.prototype.assignSocket = function(socket) {// pin("ServerResponse.prototype.assignSocket");
...
ServerResponse.prototype.detachSocket = function(socket) {// pin("ServerResponse.prototype.detachSocket");
...
ServerResponse.prototype.writeContinue = function() {// pin("ServerResponse.prototype.writeContinue");
...
ServerResponse.prototype.writeHead = function(statusCode, aa, bb) {
  ...
  var statusLine = 'HTTP/1.1 ' + statusCode.toString() + ' ' + reasonPhrase
          + CRLF;
...
function Agent(options) {
  ...
  self.on('free',
          function(socket, host, port, localAddress) {
            var name = host + ':' + port;
            if (localAddress) {
              name += ':' + localAddress;
            }

            if (!socket.destroyed && self.requests[name]
...
Agent.prototype.removeSocket = function(s, name, host, port, localAddress) {// pin("Agent.prototype.removeSocket");
  if (this.sockets[name]) {
...
ClientRequest.prototype._implicitHeader = function() {// pin("ClientRequest.prototype._implicitHeader");
  this._storeHeader(this.method + ' ' + this.path + ' HTTP/1.1\r\n', this
          ._renderHeaders());
};

ClientRequest.prototype.abort = function() {// pin("ClientRequest.prototype.abort");
...
// client
function parserOnIncomingClient(res, shouldKeepAlive) {// pin("parseOnIncomingClient");
  var parser = this;
...
ClientRequest.prototype.onSocket = function(socket) {// pin("ClientRequest.prototype.onSocket");
  var req = this;
...
ClientRequest.prototype._deferToConnect = function(method, arguments_, cb) {// pin("_deferToConnect");
...
ClientRequest.prototype.setTimeout = function(msecs, callback) {// pin("ClientRequest.prototype.setTimeout");
...
exports.request = function(options, cb) {
  ...
  if (options.protocol && options.protocol !== 'http:') { throw new Error(
          'Protocol:' + options.protocol + ' not supported.'); }
...
exports.setMaxHeaderLength = function(size) {
  if (size < 1024 && size != 0) { throw new RangeError(
          "HTTP header size can not be smaller than 1024 bytes. Set 0 to disable"); }
...
var maxHeaderSize = 32768; // commons.h
  ...
  socket.ondata = function(d, start, end, parser_response) {
    ...
    if (ret == -2) {
      if (!maxHeaderWarningShown) {
        var err = "Request is denied for security reasons. Maximum allowed header size is configured to "
                + maxHeaderSize
