...
function findTimeWithLabel(times, label) {
  if (!Array.isArray(times)) {
    jxcore.utils.console.log('Console.time/timeEnd: times must be an Array', 'red');
    return;
  }
  
  if (typeof label !== 'string') {
    jxcore.utils.console.log('Console.time/timeEnd: label must be a string', 'red');
    return;
  }
...
Console.prototype.timeEnd = function(label) {
  ...
  if (!timeEntry) {
    ...
    if (typeof timeEntry.time !== 'number') {
      jxcore.utils.console.log('Console.timeEnd: start time must be a number', 'red');
      return;
    }
