...
function setupChannel(target, channel) {
  ...
  target.send = function(message, handle) {
    if (typeof message === 'undefined') { throw new TypeError(
    ...
    if (handle) {
      ...
      if (this._handleQueue && !(message && message.cmd === 'NODE_HANDLE_ACK')) {
    ...
    if (!writeReq) {
      var er = errnoException(process._errno, 'write',
...
exports.fork = function(modulePath /* , args, options */) {
  ...
  // for native packages fork() can work without problem if modulePath is not given
  // (it just forks itself, which is used by cluster.fork() )
  if (process.isPackaged && modulePath) {
    ...
    if (!options || !options.execPath)
      throw new Error(
        ...
        "If you have jx binary installed, you may provide it with `options.execPath`.");
  }
  ...
  options.stdio = options.silent ? ['pipe', 'pipe', 'pipe', 'ipc'] : [0, 1, 2,
      'ipc'];
...
function flushStdio(subprocess) {
  subprocess.stdio.forEach(function(stream, fd, stdio) {
    if (!stream || !stream.readable || stream._consuming
            || stream._readableState.flowing) return;
    stream.resume();
  });
}
...
ChildProcess.prototype.spawn = function(options) {
  ...
  } else if (!Array.isArray(stdio)) { throw new TypeError(
          'Incorrect value of stdio option: ' + stdio); }
  ...
  stdio = stdio.reduce(function(acc, stdio, i) {
    ...
    if (stdio === 'ignore') {
      ...
      var handle = getHandleWrapType(stdio) ? stdio
