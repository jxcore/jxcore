...
JS_PUBLIC_API(bool)
JS::AddValueRootOLD(JSContext *cx, JS::Value *vp)
{
    AssertHeapIsIdle(cx);
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS::AddScriptRootOLD(JSContext *cx, JSScript **vp)
{
    AssertHeapIsIdle(cx);
    CHECK_REQUEST(cx);
    return AddScriptRoot(cx, vp, nullptr);
}
...
JS_PUBLIC_API(void)
JS::RemoveValueRootOLD(JSContext *cx, JS::Value *vp)
{
    CHECK_REQUEST(cx);
    RemoveRoot(cx->runtime(), (void *)vp);
}
                   ...
                   const JS::CompartmentOptions &options /* = JS::CompartmentOptions() */)
{
    ...
    Rooted<GlobalObject *> global(cx);
    {
        ...
        const Class * ccs = Valueify(clasp);// Valueify
        global = GlobalObject::create(cx, ccs);
    }
...
JS_PUBLIC_API(JSObject *)
JS_NewObjectForConstructorOLD(JSContext *cx, const JSClass *clasp, unsigned argc, JS::Value *val)
{
    AssertHeapIsIdle(cx);
    CHECK_REQUEST(cx);

    Value callee = MutableHandleValue::fromMarkedLocation(&val[0]);
    assertSameCompartment(cx, callee);
    RootedObject obj(cx, &callee.toObject());
    return CreateThis(cx, Valueify(clasp), obj);
}
...
JS_PUBLIC_API(bool)
JS_HasReservedSlot(JSObject *obj, uint32_t index)
{
    return obj->hasReservedSlot(index);
}
...
JS_PUBLIC_API(bool)
JS_CallFunctionValueJX(JSContext *cx, Value& obj, Value& fval, const HandleValueArray& args,
                     MutableHandleValue rval)
{
    AutoLastFrameCheck lfc(cx);

    return Invoke(cx, obj, fval, args.length(), args.begin(), rval);
}
...
JS_GetTwoByteString(JSContext *cx, JSString *str, size_t &plength) // plength -> -1 in case the source is Latin1
{
    AssertHeapIsIdleOrStringIsFlat(cx, str);
    CHECK_REQUEST(cx);
    assertSameCompartment(cx, str);
    ...
    const char16_t *data = linear->twoByteChars(nogc);
    if(!data){
      plength = 0;
      return NULL;
    }

    unsigned length = (plength + 1) * sizeof(char16_t);
    char16_t *copy = (char16_t*) JS_malloc(cx, length);
    if(!copy){
      plength = 0;
      return NULL;
    }
...
JS_EncodeStringToUTF8(JSContext *cx, HandleString str)
{
    ...
    JS::AutoCheckCannotGC nogc;
    JS::UTF8CharsZ result = linear->hasLatin1Chars()
           ? JS::CharsToNewUTF8CharsZ(cx, linear->latin1Range(nogc))
           : JS::CharsToNewUTF8CharsZ(cx, linear->twoByteRange(nogc));

    return result.c_str();
}

// JXcore extension method
JS_PUBLIC_API(char *)
JS_EncodeStringToUTF8AndLength(JSContext *cx, HandleString str, size_t &length)
{
    AssertHeapIsIdle(cx);
    CHECK_REQUEST(cx);
...
// Call this method only when the string is linear
// (just to read string length)
JS_PUBLIC_API(size_t)
JS_GetStringEncodingLengthUnsafe(JSString *str)
{
    return str->length();
}

JS_PUBLIC_API(size_t)
JS_GetStringUTF8Length(JSContext *cx, JSString *str)
{
  JSFlatString *fstr = str->ensureFlat(cx);
  if (!fstr) // return from an existing API method
    return JS_GetStringEncodingLength(cx, str);

  return JS::GetDeflatedUTF8StringLength(fstr);
}
