  ...
  *(tp = tmp) = 0;
  while ((ch = *src++) != '\0') {
    ...
    if ((pch = strchr(digits, ch)) != NULL) {
      ...
      if (saw_digit && *tp == 0) return uv_einval_;
      if (nw > 255) return uv_einval_;
      *tp = nw;
      if (!saw_digit) {
        if (++octets > 4) return uv_einval_;
        saw_digit = 1;
      }
...
static uv_err_t inet_pton6(const char *src, unsigned char *dst) {
  ...
  while ((ch = *src++) != '\0') {
    ...
    if (pch != NULL) {
      val <<= 4;
      val |= (pch - xdigits);
      if (++seen_xdigits > 4) return uv_einval_;
      continue;
    }
    if (ch == ':') {
      curtok = src;
      if (!seen_xdigits) {
        if (colonp) return uv_einval_;
        colonp = tp;
        continue;
      } else if (*src == '\0') {
        return uv_einval_;
      }
      if (tp + sizeof(uint16_t) > endp) return uv_einval_;
