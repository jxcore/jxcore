...
void clear_ares(ares_channel _ares_channel){
  int i;
  if(_ares_channel->lookups!=NULL)
    free(_ares_channel->lookups);

  if(_ares_channel->sortlist!=NULL)
    free(_ares_channel->sortlist);

  if(_ares_channel->servers!=NULL){
    free(_ares_channel->servers);
  }

  if(_ares_channel->domains!=NULL && _ares_channel->ndomains>0){
    for(i=0;i<_ares_channel->ndomains;i++)
      free(_ares_channel->domains[i]);

    free(_ares_channel->domains);
  }
}
