...
test('env shebang', function (t) {
  ...
  cmdShim(from, to, function(er) {
    ...
    t.equal(fs.readFileSync(to, 'utf8'),
            ...
            "\n")
    t.equal(fs.readFileSync(to + '.cmd', 'utf8'),
            "@IF EXIST \"%~dp0\\jx.exe\" (\r"+
            "\n  \"%~dp0\\jx.exe\"  \"%~dp0\\from.env\" %*\r"+
            "\n) ELSE (\r"+
            "\n  @SETLOCAL\r"+
...
test('env shebang with args', function (t) {
  ...
  cmdShim(from, to, function(er) {
    ...
    t.equal(fs.readFileSync(to, 'utf8'),
            ...
            "\n"+
            "\nif [ -x \"$basedir/jx\" ]; then"+
            "\n  \"$basedir/jx\"  --expose_gc \"$basedir/from.env.args\" \"$@\""+
            "\n  ret=$?"+
            "\nelse "+
            "\n  jx  --expose_gc \"$basedir/from.env.args\" \"$@\""+
            "\n  ret=$?"+
            "\nfi"+
            "\nexit $ret"+
            "\n")
    t.equal(fs.readFileSync(to + '.cmd', 'utf8'),
            "@IF EXIST \"%~dp0\\jx.exe\" (\r"+
            "\n  \"%~dp0\\jx.exe\"  --expose_gc \"%~dp0\\from.env.args\" %*\r"+
            "\n) ELSE (\r"+
            "\n  @SETLOCAL\r"+
            "\n  @SET PATHEXT=%PATHEXT:;.JS;=;%\r"+
            "\n  jx  --expose_gc \"%~dp0\\from.env.args\" %*\r"+
            "\n)")
    t.end()
  })
})
