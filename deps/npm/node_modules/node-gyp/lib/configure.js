...
function configure (gyp, argv, callback) {
  ...
  function createConfigFile (err) {
    ...
    console.log("Target arch", variables.target_arch);

    var platform = process.platform == "darwin" ? "mac" : process.platform;
    variables.OS = gyp.opts.dest || platform;

    // correct OS for windows (platform == win32 under windows but gyp needs win)
    if (variables.OS == "win32")
      variables.OS = "win";

    var osInfo = jxcore.utils.OSInfo();

    if (!variables["node_win_onecore"])
      variables["node_win_onecore"] = 0;

    if (process.versions.sm) {
      defaults.defines.push("JS_ENGINE_MOZJS");
      if (variables.target_arch == "x64") {
        var arr64 = ['JS_PUNBOX64'];
        defaults.defines = defaults.defines.concat(arr64);
      } else if (variables.target_arch == "ia32") {
        var arr32 =  ['JS_NUNBOX32', 'JS_CPU_X86=1'];
        defaults.defines = defaults.defines.concat(arr32);
      } else if (variables.target_arch == "arm") {
        var arr32 =  ['JS_NUNBOX32', 'JS_CPU_ARM=1'];
        defaults.defines = defaults.defines.concat(arr32);
      }
      defaults.cflags.push("-std=c++0x");
    ...
    } else if (process.versions.chakra) {
      defaults.defines.push("JS_ENGINE_CHAKRA");
      // Chakra uses V8 3.28 proxy
      defaults.defines.push("JS_ENGINE_V8");
      defaults.defines.push("V8_IS_3_28");
    } else if (parseFloat(process.versions.v8) <= 3.15) {
      defaults.defines.push("JS_ENGINE_V8");
      defaults.defines.push("V8_IS_3_14");
    } else {
      defaults.defines.push("JS_ENGINE_V8");
      defaults.defines.push("V8_IS_3_28");
    }
